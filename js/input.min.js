	...
	acf.conditional_logic = {
		...
		render_field : function( key ){
			...
			$targets.each(function(){
				...
				{
					// clear classe
					$target.removeClass('hidden-by-conditional-logic');
					...
					// hook
					acf.do_action('conditional_logic_show', $target );
										
				}
				else
				{
					// add classe
					$target.addClass('hidden-by-conditional-logic');
					...
					// hook
					acf.do_action('conditional_logic_hide', $target );
				}
		...
		get_visibility : function( $target, rule ){
			...
			console.log('get_visibility');
			// vars
			var $trigger = acf.get_fields({key : rule.field});
			
			
			$trigger.each(function(){
				
				// bail early if not a sub field
				if( !acf.is_sub_field( $(this) ) )
				{
					return;
				}
				
				
				// if $(this) sub field is a sibling of the target, this is the $trigger that is relevant
				if( $(this).siblings('.acf-field[data-key="' + acf.get_data($target, 'key') + '"]').exists() )
				{
					$trigger = $(this);
					return false;
				}
